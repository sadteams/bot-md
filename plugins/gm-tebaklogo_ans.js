...
handler.before = async function (m) {
  ...
  if (m.quoted.id == this.logoquiz[id][0].id) {
    ...
    if (['.oncs', 'Bantuan', ''].includes(m.text)) return !0
    if (m.text.toLowerCase() == json.jawaban.toLowerCase()) {
      global.db.data.users[m.sender].exp += this.logoquiz[id][2]
      await this.sendButton(m.chat, benar + `${this.logoquiz[id][2]} XP`, wm, 'Logo Quiz', '.logoquiz', m)
...
handler.before = async function (m) {
  let id = m.chat
  if (!m.quoted || !m.quoted.fromMe || !m.quoted.isBaileys || !/Ketik.*oncs/i.test(m.quoted.contentText)) return !0
  this.logoquiz = this.logoquiz ? this.logoquiz : {}
  if (!(id in this.logoquiz)) return m.reply('Soal itu telah berakhir')
  if (m.quoted.id == this.logoquiz[id][0].id) {
    let json = JSON.parse(JSON.stringify(this.logoquiz[id][1]))
    ...
    if (m.text.toLowerCase() == json.jawaban.toLowerCase()) {
      global.db.data.users[m.sender].exp += this.logoquiz[id][2]
      await this.sendButton(m.chat, `*Benar!* +${this.logoquiz[id][2]} XP`, '', 'Logoquiz', '.logoquiz', m)
      clearTimeout(this.logoquiz[id][3])
      delete this.logoquiz[id]
