import { smsg } from './system/simple.js'
import { db, opts, loadDatabase } from './system/database.js'
import { owner, mods, Prefix, prems, APIKeys, wm, dfail } from './settings.js'
import printMessage from './system/print.js'
...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        const isROwner = [conn.decodeJid(conn.user.id), ...owner.map(([number]) => number)].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                try {
                    ...
                    if (e) {
                        ...
                        if (e.name)
                            for (let [jid] of owner.filter(([number, isCreator, isDeveloper]) => isDeveloper && number)) {
