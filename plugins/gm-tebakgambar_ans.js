...
handler.before = async function (m) {
  ...
  if (m.quoted.id == this.tebakgambar[id][0].id) {
    ...
    if (m.text.toLowerCase() == json.jawaban.toLowerCase()) {
      ...
      await this.sendButton(m.chat, `Selamat *@${m.sender.split(`@`)[0]}* jawaban kamu benar, bonus +${this.tebakgambar[id][2]} XP`, wm, 'Tebak Gambar', '.tebakgambar', m, {mentions: [m.sender]})
